...
2. Load plugin: Settings → Plugins → Load unpacked plugin → `/Users/niyaro/Documents/Code/Logseq/logseq-checklist`
...
├── src/
│   ├── index.ts       # Plugin entry, DB.onChanged setup
│   ├── progress.ts    # Core algorithm (recursive counting)
│   ├── events.ts      # Event handling (filter, debounce, find parent)
│   ├── content.ts     # Content manipulation (add/remove indicators)
│   └── types.ts       # TypeScript interfaces
├── dist/              # Build output (generated by Vite)
├── package.json       # v0.1.14
├── CHANGELOG.md       # Full version history
├── README.md          # User documentation
└── GETTING_STARTED.md # Original planning doc
```
...
1. ✅ Update version number with EVERY build iteration
2. ✅ Update CHANGELOG.md for each version
3. ✅ Commit to git after each version
4. ❌ NOT pushed to remote yet (local repository only)
...
**What works:**
- ✅ Checkbox detection (property changes)
...
**What's broken:**
- ❌ Tag detection (datascript query syntax error)
- ❌ Finding parent checklist blocks
...
**Ready to continue!** Load this file tomorrow and we'll debug the DB.onChanged filtering logic. 🚀
